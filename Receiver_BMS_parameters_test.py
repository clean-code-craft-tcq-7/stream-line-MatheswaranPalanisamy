...
class BMS_parameters_Receiver(unittest.TestCase):
    ...
    def test_find_simple_moving_average(self):
        ...
        mock_bms_parameters = {'ChargeRate': mock_charge_rate, 'Temperature': mock_temperature}
        self.assertEqual(find_simple_moving_average(mock_bms_parameters), (0.24000000000000005, 25.8))

    def test_find_max_and_min_values_of_parameters(self):
        mock_charge_rate = [0.1, 0.2, 0.4, 0.0, 0.5]
        mock_temperature = [24.0, 26.0, 23.0, 27.0, 29.0]
        mock_bms_parameters = {'ChargeRate': mock_charge_rate, 'Temperature': mock_temperature}
        self.assertEqual(find_max_and_min_values_of_parameters(mock_bms_parameters), {'ChargeRate': {'max': 0.5, 'min': 0.0}, 'Temperature': {'max': 29.0, 'min': 23.0}})
    ...
    def test_receive_bms_paramters(self):
        ...
        self.assertEqual(receive_bms_paramters(sender_output), {'ChargeRate': charge_rate, 'Temperature': temperature})
        
    @patch('builtins.print')
    def test_print_max_and_min_values_of_parameters(self, mock_print):
        mock_max_and_min = {'ChargeRate': {'max': 0.5, 'min': 0.0}, 'Temperature': {'max': 29.0, 'min': 23.0}}
        print_max_and_min_values_of_parameters(mock_max_and_min)
        mock_print.assert_called_with("max and min values of parameters: ",{'ChargeRate': {'max': 0.5, 'min': 0.0}, 'Temperature': {'max': 29.0, 'min': 23.0}})
